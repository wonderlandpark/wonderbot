module.exports.execute = async (client, message, locale, embed, tools, knex, props, data) => {
    if((await knex.select('*').from('users').where({id : message.author.id}))[0].money == 0) return message.reply(locale.commands.allin.nomoney)
   if(!message.guild.me.hasPermission('ADD_REACTIONS')){
       ...
       msg.then(async m=> {m.react('💸'); m.awaitReactions(filter, { max: 1, time : 10000, error : ['time']})
        ...
        if(collected.size == 0) {
            ...
            g.then(async gg=> {gg.edit(gg.content + '\n'+locale.commands.allin.then)
               setTimeout(async function(){
                const res = [true,false].random()
                const u = (await knex.select('*').from('users').where({id : message.author.id}))[0]
                const money = u.money
                const multi = u.multiples+1

                if(res){
                    await knex.update({money : money*2*multi, multiples : multi}).from('users').where({id : message.author.id})
                    message.reply(locale.commands.allin.success.bind({money : money*2*multi, mul : 2*multi, n : multi}))
                    data.action.splice(data.action.indexOf(message.data.id), 1)
                    
                }
                else {
                    await knex.update({money : 0, multiples : 0}).from('users').where({id : message.author.id})
