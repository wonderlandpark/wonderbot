module.exports.execute = async (
    ...
    const reason = message.data.arg2 ? message.data.arg2 : locale.commands.warn.none
    ...
    if (limit <= warndata[user.id].count) {
      ...
      .then(async ()=> {
        ...
        embed.addField(locale.commands.ban.reason, locale.commands.ban.reasonDesc.bind({reason: locale.commands.warn.auto }))
        await knex('guilds').update({warn: JSON.stringify(warndata)}).where({id: message.guild.id})
        message.reply(embed)
        tools.bot.modlog(client, message.guild.id, embed)
      })
