...
module.exports.execute = async (
    ...
    const MappedResult = Result.progresses.map(l => ({ desc: l.description, day: new Date(l.time).textFormat('YYYYMMDD'), time: new Date(l.time), location: l.location, status: l.status }));
    var json = {};
    ...
    for (let key of Object.keys(json)) {
      embed.addField(moment(key, 'YYYYMMDD').format('YYYY - MM - DD'), json[key].map(l => `${locale.commands.delivery.status[l.status.id] ? locale.commands.delivery.status[l.status.id] : locale.commands.delivery.status.unknown} -  **[${l.location.name}]** **${l.time.textFormat('HH:mm')}** - ${l.desc}`));
    }
