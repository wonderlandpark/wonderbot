...
module.exports.execute = async (client, message, locale, embed, tools) => {
  async function epage (status, m, p) {
    ...
    console.log(p)
    for(i=(p-1)*5; i<p*5; i++){
      if(status.stores[i]) embed.addField(status.stores[i].name, `${(statusCode[status.stores[i]['remain_stat']] ?  tools.lib.emojis[status.stores[i]['remain_stat']]+ ' ' + statusCode[status.stores[i]['remain_stat']] : '❔ 알 수 없음')}\n  주소: \`${status.stores[i].addr}\`\n좌표: \`${status.stores[i].lat}, ${status.stores[i].lng}\` - [구글 맵](https://www.google.co.kr/maps/search/${status.stores[i].lat}+${status.stores[i].lng})`)
    }
  ...
  status.stores.chunkArray(5).forEach(s=> {
    ...
    embed.addField('오늘 마스크는?', maskDay[new Date().getDay()])
    s.forEach(g=> {
      embed.addField(g.name, `${(statusCode[g['remain_stat']] ?  tools.lib.emojis[g['remain_stat']]+ ' ' + statusCode[g['remain_stat']] : '❔ 알 수 없음')}\n  주소: \`${g.addr}\`\n좌표: \`${g.lat}, ${g.lng}\` - [구글 맵](https://www.google.co.kr/maps/search/${g.lat}+${g.lng})`)
    })
