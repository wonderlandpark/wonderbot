module.exports.execute = async (
  ...
  if (user.hasPermission(['ADMINISTRATOR'])) return message.reply(locale.commands.unwarn.alsoPerm)
  if (!warndata[user.id] || warndata[user.id].count < 1 ) return message.reply(locale.commands.unwarn.noWarn)
  else {
    ...
    await message.reply({content: locale.commands.unwarn.select, embed: warns})
  ...
  embed.addField(
    ...
    locale.commands.unwarn.userDesc.bind({ user: user.user, tag: user.user.tag })
  )
  embed.addField(
