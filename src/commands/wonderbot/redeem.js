module.exports.execute = async ( client, message, locale, embed, tools, knex ) => {
    ...
    const coupon = await knex('coupon').where({ id }).orWhere('expire', '>', Math.round(new Date()/1000))
    ...
    if(coupon[0].type === 'premium') {
        if((coupon[0].option & 1) && user.permium > Math.round(new Date()/1000)) return message.reply('**죄송합니다 해당 쿠폰은 사용하실 수 없습니다.**\n\n해당 쿠폰은 원더봇 프리미엄이 적용되있지 않은 경우에만 사용하실 수 있습니다.')
        ...
        await knex('users').update({ premium: user.permium > Math.round(new Date() / 1000) ? user.permium + coupon[0].value : Math.round(new Date() / 1000) + coupon[0].value }).where({ id: message.author.id })

        return message.reply(`**쿠폰이 적용되었습니다**\n\n적용된 쿠폰: 원더봇 프리미엄 ${Math.round(coupon[0].value / (60*1000*60))}일`)
