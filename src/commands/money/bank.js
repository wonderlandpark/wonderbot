...
module.exports.execute = async (
    ...
    if(message.guild.id !== '542599372836438016' && message.channel.id !== '743718965934686271') return message.reply('해당 기능은 베타 테스트중인 기능입니다. 사용하실 수 없습니다.')
    ...
    if(!message.data.args) {
        ...
        const reqMoney = message.data.arg[1] === '최대' ? avail : Number(message.data.arg[1])
        ...
        m.awaitReactions((reaction, user) => reaction.emoji.name === '✅' && user.id === message.author.id, { max: 1, time: 10000, error: ['time']}).then(async collected => {
            ...
            await knex('users').where({ id: message.author.id }).update({ money: +u.money + Number(reqMoney), loan_money: u.loan_money + reqMoney*(1+u.loan_lvl*0.1), loan_date: time, action: 0 })
        ...
        const reqMoney = message.data.arg[1] === '전부' ? u.loan_money : Number(message.data.arg[1])
        if(isNaN(reqMoney) || !Number.isInteger(reqMoney)) return message.reply('올바르지 않은 금액입니다. 올바른 정수를 입력해주세요.')
