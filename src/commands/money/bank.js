...
module.exports.execute = async (
    ...
    if(!message.data.args) {
        ...
        m.awaitReactions((reaction, user) => reaction.emoji.name === '✅' && user.id === message.author.id, { max: 1, time: 10000, error: ['time']}).then(async collected => {
            ...
            await knex('users').where({ id: message.author.id }).update({ money: Math.round(+u.money - Number(reqMoney)), loan_money: u.loan_money - reqMoney, action: 0, loan_lvl, loan_point: loan_lvl !== u.loan_lvl ? 0 : point })
