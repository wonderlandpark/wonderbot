...
module.exports.execute = async (
    ...
    if(!message.data.args) {
        ...
        const reqMoney = Math.round(message.data.arg[1] === '최대' ? avail : Number(message.data.arg[1]))
        ...
        const reqMoney = Math.round(message.data.arg[1] === '전부' ? u.loan_money : Number(message.data.arg[1]))
        if(isNaN(reqMoney) || !Number.isInteger(reqMoney)) return message.reply('올바르지 않은 금액입니다. 올바른 정수를 입력해주세요.')
